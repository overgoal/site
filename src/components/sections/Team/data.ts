export const teamData = [
  {
    name: "Matias Grunwaldt",
    role: "CEO & Dev",
    image: "/team/mate.png",
    description:
      "Launched El Super DT, Uruguay's 1st fantasy football game. CTO at Eyecue Insights, Co-founder at Lambda Class Game Studio. 2024 Dojo NY Residency participant.",
    socials: {
      twitter: "https://x.com/matigru93",
    },
  },
  {
    name: "Bautista Berto",
    role: "CTO & Dev",
    image: "/team/baute.png",
    description:
      "Ex–Lead Game Dev at Lambda Class Game Studio. Built multiple mobile games & Web3 apps, Hackathon winner & creative coder at Basement Studio.",
    socials: {
      twitter: "https://x.com/berto_bau",
      linkedin: "https://www.linkedin.com/in/bautista-berto/",
      github: "https://github.com/BertovDev",
      website: "https://bautistaberto.vercel.app/",
    },
  },
  {
    name: "Alex Mack",
    role: "Producer & Game Design",
    image: "/team/alex.png",
    description:
      "Shipped 30+ games as Exec Producer & Systems Designer. Ex–EA, Namco, Sony, Atari, Cryptic, Forte.io, ngmoco. Pioneer in freemium & blockchain game design. 2024 Dojo NY Residency participant.",
    socials: {
      twitter: "https://x.com/tulpalab_",
    },
  },
  {
    name: "Ger Gahn",
    role: "Art Director",
    image: "/team/ger.png",
    description:
      "2D Animator at Lambda Forge Studio. Ex–Lead 3D Artist at Lambda Studio. Versatile game artist: 2D, 3D, VFX & SFX. Mentored by top industry veterans.",
    socials: {
      twitter: "https://x.com/GerGahn",
    },
  },
];
